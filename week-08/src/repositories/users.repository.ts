...
export async function findByEmail(email: string) { return User.findOne({ email }).lean(); }
export async function findById(id: string) { return User.findById(id).lean(); }
export async function create(data: { email: string; password: string; name: string }) {
  const user = await User.create(data); return user.toObject();
}
