...
export async function findAll(page: number, limit: number) {
  const [data, total] = await Promise.all([
    Benefit.find().populate('createdBy', 'name email role').skip((page - 1) * limit).limit(limit).sort({ createdAt: -1 }).lean(),
...
export async function findById(id: string) {
  ...
  return Benefit.findById(id).populate('createdBy', 'name email role').lean();
}
export async function create(dto: CreateBenefitInput, userId: string) {
  try {
    const doc = await Benefit.create({ ...dto, createdBy: userId });
    return Benefit.findById(doc._id).populate('createdBy', 'name email role').lean();
  } catch (err: unknown) {
...
export async function update(id: string, dto: UpdateBenefitInput) {
  if (!mongoose.isValidObjectId(id)) throw new AppError(400, `Id '${id}' no es válido`);
  try {
    return Benefit.findByIdAndUpdate(id, dto, { new: true, runValidators: true }).populate('createdBy', 'name email role').lean();
  } catch (err: unknown) {
    if (err instanceof mongoose.mongo.MongoServerError && err.code === 11000)
