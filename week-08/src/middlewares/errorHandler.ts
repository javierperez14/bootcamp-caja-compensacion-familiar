...
export function errorHandler(err: unknown, _req: Request, res: Response, _next: NextFunction): void {
  if (err instanceof ZodError) {
    res.status(400).json({ error: 'Validation Error', message: 'Datos inválidos', issues: err.issues.map(i => ({ path: i.path, message: i.message })) });
    return;
  }
  ...
  if (err instanceof mongoose.Error.CastError) {
    res.status(400).json({ error: 'Bad Request', message: `Valor '${err.value}' inválido para '${err.path}'` });
    return;
  }
